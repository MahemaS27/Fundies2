...
abstract class ANode<T> {
  ...
  // gets this nodes data, if its a sentinel just returns "no data for sentinel"
  T getData() {
    throw new RuntimeException("no data for sentinel");
  }
...
class Sentinel<T> extends ANode<T> {
  ...
  // finds the item specified by the pred ; will recur back to sentinel if not
  // found
...
class ExamplesDeque {
  ...
  void testremoveFromTail(Tester t) {
    ...
    t.checkExpect(deque3.removeFromTail(), 20);
  }
